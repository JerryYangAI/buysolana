...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return pageMetadata(locale, "podcastTitle", "/podcast", messages.podcast.description);
}
...
export default async function PodcastPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const messages = await getMessages(locale);
  const items = await listContent("podcast", locale);

  return (
    <div className="space-y-4">
      <PageHero title={messages.podcast.title} description={messages.podcast.description} />
      <div className="grid gap-3">
        {items.map((item) => (
          <Link
            key={item.slug}
            href={withLocale(locale, `/podcast/${item.slug}`)}
